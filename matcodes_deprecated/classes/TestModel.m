...
%             plot(365,PopAtmp.alive,'DisplayName','alive');
        ...
        function HeatmapPlot(self, changing_variable, pop)
            ...
            if pop == 1
                ...
                if changing_variable == 4
                    imagesc(self.PopA.plong.mean, self.PopA.plong.std, reshape(self.TIall, length(self.PopA.plong.std), []));
                    colorbar;
                    xlabel('Population A plant longevity');
                    ylabel('stdev of Population A plant longevity');
                end
            elseif pop == 2
                if changing_variable == 1
                ...
                if changing_variable == 4
                    imagesc(self.PopB.plong.mean, self.PopB.plong.std, reshape(self.TIall, length(self.PopB.plong.std), []));
                    colorbar;
                    xlabel('Population B plant longevity');
                    ylabel('stdev of Population B plant longevity');
                end
            end
